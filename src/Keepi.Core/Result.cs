...
{
    bool TrySuccess([NotNullWhen(returnValue: false)] out TError? error);
...
public class MaybeErrorResult<TError> : IMaybeErrorResult<TError>
{
    ...
    {
        return new MaybeErrorResult<TError>(succeeded: true, error: default);
    }

    public static IMaybeErrorResult<TError> CreateFailure(TError value)
    {
        return new MaybeErrorResult<TError>(succeeded: false, error: value);
    }
...
public class ValueOrErrorResult<TSuccess, TError> : IValueOrErrorResult<TSuccess, TError>
{
    private readonly bool succeeded;
        ...
        get
        {
            if (!succeeded || success == null)
            {
                throw new InvalidOperationException(
                    "Non succesful result cannot return a success value"
                );
            }
