 ...
 : IUpdateUserEntryCategoryUseCase
{
    ...
    CancellationToken cancellationToken)
  {
    ...
    {
      return MaybeErrorResult<UpdateUserEntryCategoryUseCaseError>.CreateFailure(UpdateUserEntryCategoryUseCaseError.MalformedName);
    }

    if (!UserEntryCategoryEntity.IsValidActiveDateRange(from: activeFrom, to: activeTo))
    {
      return MaybeErrorResult<UpdateUserEntryCategoryUseCaseError>.CreateFailure(UpdateUserEntryCategoryUseCaseError.InvalidActiveDateRange);
    }
    ...
    if (error == UpdateUserEntryCategoryError.UserEntryCategoryDoesNotExist ||
      error == UpdateUserEntryCategoryError.UserEntryCategoryBelongsToOtherUser)
    {
      return MaybeErrorResult<UpdateUserEntryCategoryUseCaseError>.CreateFailure(UpdateUserEntryCategoryUseCaseError.UnknownUserEntryCategory);
    }

    if (error == UpdateUserEntryCategoryError.DuplicateName)
    {
      return MaybeErrorResult<UpdateUserEntryCategoryUseCaseError>.CreateFailure(UpdateUserEntryCategoryUseCaseError.DuplicateName);
    }

    return MaybeErrorResult<UpdateUserEntryCategoryUseCaseError>.CreateFailure(UpdateUserEntryCategoryUseCaseError.Unknown);
  }
}