...
{
  Task<GetUserEntriesForWeekUseCaseOutput> Execute(int userId, int year, int weekNumber, CancellationToken cancellationToken);
}

internal class GetUserEntriesForWeekUseCase(
  IGetUserEntriesForDates getUserEntriesForDates)
  : IGetUserEntriesForWeekUseCase
{
  public async Task<GetUserEntriesForWeekUseCaseOutput> Execute(int userId, int year, int weekNumber, CancellationToken cancellationToken)
  {
  ...
  private static GetUserEntriesForWeekUseCaseOutputDay GetDayForEntitiesOnDate(DateOnly date, UserEntryEntity[] entities)
  {
    return new(
      Entries: entities
        .Where(e => e.Date == date)
