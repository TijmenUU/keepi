...
const props = defineProps<{
  ...
  editingProject: IGetAllProjectsResponse['projects'][0]
}>()
...
const { r$ } = useRegle(formValues, {
  name: {
    ...
    unique: withMessage((value) => {
      ...
      return !props.existingProjects.some(
        (p) => p.id !== props.editingProject.id && p.name === value,
      )
...
const onOpen = (ev: Event) => {
  ...
  formValues.value = getFormValues(props.editingProject, users)
  r$.$reset()
...
const onSubmit = async () => {
  ...
  try {
    ...
    if (valid) {
      await apiClient
        .updateProject(props.editingProject.id, {
          name: data.name,
          enabled: data.enabled,
          userIds: data.users.filter((u) => u.value).map((u) => u.id),
          invoiceItems: data.invoiceItems.map((i) => ({ id: i.originalId, name: i.name })),
        })
        .match(
          () => {
...
function getFormValues(
  project: IGetAllProjectsResponse['projects'][0],
  users: IGetAllUsersResponse['users'],
) {
  let invoiceItemUniqueId = 0
  const invoiceItems: { id: number; originalId?: number; name: string }[] =
    project.invoiceItems.map((i) => ({ id: invoiceItemUniqueId++, originalId: i.id, name: i.name }))
  if (invoiceItems.length < 1) {
    invoiceItems.push({ id: invoiceItemUniqueId++, name: '' })
  }
  ...
  return {
    ...
    users: users.map((u) => ({
      ...
      name: u.id.toString(),
      label: u.name,
      value: project.users.some((pu) => pu.id === u.id),
    })),
  }
}
</script>
    ...
    <div>
      <DialogContent
        ...
        <DialogHeader>
          ...
          <DialogDescription>
            Let op dat het verwijderen van gebruikers en/of factuurposten kan leiden tot data
            verlies.
          </DialogDescription>
        </DialogHeader>
