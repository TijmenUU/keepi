...
{
  async Task<IMaybeErrorResult<OverwriteUserEntriesForDatesError>> IOverwriteUserEntriesForDates.Execute(
    int userId,
    DateOnly[] dates,
    ...
    CancellationToken cancellationToken)
  {
    ...
    try
    {
      ...
      await databaseContext.UserEntries
        .Where(ue => ue.UserId == userId && dates.Contains(ue.Date))
        .ExecuteDeleteAsync(cancellationToken: cancellationToken);

      foreach (var entry in userEntries)
      {
        databaseContext.Add(new UserEntryEntity
        {
          UserId = userId,
    ...
    catch (Exception ex)
    {
      logger.LogError(
        ...
        dates.LastOrDefault());
      return MaybeErrorResult<OverwriteUserEntriesForDatesError>.CreateFailure(OverwriteUserEntriesForDatesError.Unknown);
    }
    finally
    {
      if (databaseContext.Database.CurrentTransaction != null)
      {
        await databaseContext.Database.RollbackTransactionAsync(cancellationToken: CancellationToken.None);
      }
    }
  }

  async Task<Core.Entries.UserEntryEntity[]> IGetUserEntriesForDates.Execute(int userId, DateOnly[] dates, CancellationToken cancellationToken)
  {
    Debug.Assert(dates.Length > 0);

    return (await databaseContext.UserEntries
      .Where(ue => ue.UserId == userId && dates.Contains(ue.Date))
