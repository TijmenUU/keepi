...
public class EntryCategoryEntityTests
{
  [Theory]
  [InlineData("a")]
  [InlineData("1")]
  ...
  {
    EntryCategoryEntity.IsValidName(name).ShouldBeTrue();
  }
  ...
  {
    EntryCategoryEntity.IsValidName(name).ShouldBeFalse();
  }
