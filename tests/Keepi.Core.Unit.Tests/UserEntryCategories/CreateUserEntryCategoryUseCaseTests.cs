...
public class CreateUserEntryCategoryUseCaseTests
{
  ...
  public async Task Execute_stores_expected_entity_for_valid_input()
  {
    var context = new TestContext()
      .WithSuccesfulUserEntryCategoryStore(new UserEntryCategoryEntity(
          id: 13,
          name: "Dev",
          enabled: true,
    ...
    context.StoreUserEntryCategoryMock.Verify(x => x.Execute(
      42,
      "Dev",
      true,
  ...
  class TestContext
  {
    public Mock<IStoreUserEntryCategory> StoreUserEntryCategoryMock { get; } = new(MockBehavior.Strict);

    public TestContext WithSuccesfulUserEntryCategoryStore(UserEntryCategoryEntity returnedEntity)
    {
      StoreUserEntryCategoryMock
        .Setup(x => x.Execute(
          ...
          It.IsAny<DateOnly?>(),
          It.IsAny<CancellationToken>()))
        .ReturnsAsync(ValueOrErrorResult<UserEntryCategoryEntity, StoreUserEntryCategoryError>.CreateSuccess(returnedEntity));
    ...
    public TestContext WithFailedUserEntryCategoryStore(StoreUserEntryCategoryError error)
    {
      StoreUserEntryCategoryMock
        .Setup(x => x.Execute(
          ...
          It.IsAny<DateOnly?>(),
          It.IsAny<CancellationToken>()))
        .ReturnsAsync(ValueOrErrorResult<UserEntryCategoryEntity, StoreUserEntryCategoryError>.CreateFailure(error));
