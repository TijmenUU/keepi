...
public class UserEntryCrudWorkflow(KeepiWebApplicationFactory applicationFactory)
{
  ...
  public async Task Create_read_update_delete_entries_test()
  {
    ...
    var developmentUserEntryCategoryId = await CreateUserEntryCategory(client: client, name: "Dev");
    var administrationUserEntryCategoryId = await CreateUserEntryCategory(client: client, name: "Administratie");
    ...
    savedWeek.ShouldBeEquivalentTo(new GetWeekUserEntriesResponse(
      Monday: new(
        Entries: [
          new (EntryCategoryId: developmentUserEntryCategoryId, Minutes: 60, Remark: "Nieuwe feature"),
          new (EntryCategoryId: administrationUserEntryCategoryId, Minutes: 45, Remark: "Project Flyby"),
        ]
      ),
      Tuesday: new(
        Entries: [
          new (EntryCategoryId: developmentUserEntryCategoryId, Minutes: 30, Remark: null),
        ]
      ),
      Wednesday: new(
        Entries: [
          new (EntryCategoryId: administrationUserEntryCategoryId, Minutes: 15, Remark: null),
        ]
      ),
      Thursday: new(Entries: []),
      Friday: new(Entries: []),
    ...
    savedWeek.ShouldBeEquivalentTo(new GetWeekUserEntriesResponse(
      Monday: new(
        Entries: [
          new (EntryCategoryId: developmentUserEntryCategoryId, Minutes: 61, Remark: "Nieuwe feature1"),
          new (EntryCategoryId: administrationUserEntryCategoryId, Minutes: 46, Remark: "Project Flyby1"),
        ]
      ),
      Tuesday: new(
        Entries: [
          new (EntryCategoryId: developmentUserEntryCategoryId, Minutes: 31, Remark: "1"),
        ]
      ),
      Wednesday: new(
        Entries: [
          new (EntryCategoryId: administrationUserEntryCategoryId, Minutes: 16, Remark: "2"),
        ]
      ),
      Thursday: new(Entries: []),
      Friday: new(Entries: []),
  ...
  private static async Task UpdateWeekEntries(HttpClient client, int year, int weekNumber, PutUpdateWeekUserEntriesRequest request)
  {
    var httpResponse = await client.PutAsJsonAsync(
      requestUri: $"/api/user/entries/year/{year}/week/{weekNumber}",
      value: request);
    httpResponse.StatusCode.ShouldBe(System.Net.HttpStatusCode.Created);
    httpResponse.Headers.Location?.OriginalString.ShouldBe($"/api/user/entries/year/{year}/week/{weekNumber}");
  }

  private static async Task<GetWeekUserEntriesResponse> GetWeekEntries(HttpClient client, int year, int weekNumber)
  {
    var httpResponse = await client.GetAsync(requestUri: $"/api/user/entries/year/{year}/week/{weekNumber}");
